...
57. **Un SFX de impacto sintetizado con tonos puros (aunque sea fundamental+armónico, ej. una marimba) se percibe "blando"/sintético hasta que se le agrega un TRANSIENTE al inicio — un burst de ruido de solo ~10ms, ANTES/junto con el cuerpo tonal, es lo que el oído interpreta como el "click" de contacto real (mazo/objeto contra superficie).** Confirmado con investigación (técnica estándar de sound design para percusión: capa de ruido corta al ataque + filtro para el cuerpo tonal) tras dos rondas previas de ajuste de tono que no convencieron al usuario — el problema no era la frecuencia/duración elegida, sino la falta de ese transiente. Ver `tools/gen_assets.py::sfx_totopo_crunch()`: `_env(_noise(0.012, ...), 0.0005, 0.01)` mezclado con las capas tonales existentes vía `_mix()`. **Además**: dos SFX con picos de amplitud CASI IDÉNTICOS (medidos en la muestra final, no solo en el parámetro `amp` de la función generadora) pueden sonar con volumen PERCIBIDO muy distinto si difieren en frecuencia — un tono puro agudo (~1kHz+, zona de máxima sensibilidad del oído humano, curvas isofónicas) se percibe más fuerte que un tono grave/compuesto a igual pico de amplitud y hasta con varios dB menos de ganancia nominal. Al balancear el volumen relativo de dos SFX, no alcanza con bajar dB por sensación — verificar el pico real de la muestra generada (`wave`/`struct` en Python) y, si uno de los dos es un tono agudo puro, compensar también bajando su `pitch_scale` (aleja la frecuencia de la zona sensible), no solo su `volume_db`. **Además (v2, "el láser sobresale en volumen aunque baje el volumen general")**: al auditar el balance de TODOS los SFX de un proyecto, medir RMS (no solo pico) de cada `.wav` real — RMS es mejor proxy de volumen PERCIBIDO porque captura cuánto tiempo pasa el sonido cerca de su pico, no solo el punto más alto. Cualquier SFX generado con `_mix()` que nunca haya recibido el rescale manual explícito (ver el bug de "siempre normaliza a 90%" arriba) queda con su volumen "de fábrica" muy por encima de los SFX hermanos que sí fueron ajustados — hay que revisar CADA `.wav` del proyecto, no asumir que arreglar uno arregla el resto. Además, un SFX ligado a una entidad PERSISTENTE que puede retocarse/reactivarse muchas veces seguidas (ej. un power-up que no se destruye al usarse) necesita, a igual RMS nominal, sonar proporcionalmente MÁS silencioso que un SFX de un solo uso — cada activación crea un `AudioStreamPlayer` nuevo que se solapa con los anteriores si el original no terminó, así que el volumen final percibido depende de la frecuencia de activación tanto como del volumen de una sola instancia (mismo principio que ya aplicaba al rebote contra pared). Acortar la duración del clip (menos ventana de solape) es tan efectivo como bajar el volumen.
...
59. **Cuando una entidad tiene una posición LÓGICA (`grid_pos`, usada para decidir qué fila/columna/celda afecta) separada de su posición VISUAL (un `Tween` que anima `position` en pantalla), cualquier código que actualice una TIENE que actualizar la otra — copiar el bucle de movimiento para un tipo de entidad nuevo sin copiar esa línea deja el dato lógico congelado para siempre, aunque se vea moverse perfectamente en pantalla.** Bug real reportado jugando: "el láser horizontal siempre afecta solo a la primera línea (de arriba), independientemente en qué posición esté" — `BoardManager._shift_down()` actualiza `grid_pos` en cada bloque al bajarlo una fila (`node.set(&"grid_pos", new_key)`) junto con el tween visual, pero el bucle equivalente para los power-ups/íconos (agregado después, mismo patrón) solo tenía el tween — nunca el `.set()` de `grid_pos`. El ícono se veía bajar en pantalla con total normalidad (el tween sí corría), pero la lógica que decide "qué fila disparo" seguía usando el valor de spawn (fila 0) para siempre. Sin error, sin warning — indistinguible de un bug de diseño/probabilidad hasta que alguien nota que SIEMPRE es la misma fila sin importar cuánto haya avanzado la partida. Regla general: si copias un bucle de "mover N entidades una posición" para un tipo de entidad nuevo, verificar explícitamente que se actualiza CUALQUIER propiedad lógica que otro sistema lea después (no solo la visual) — un test que solo verifique "se ve en la posición visual correcta" no atrapa esto; hace falta verificar el dato lógico (`node.get(&"grid_pos")`) después de la operación, no solo la posición en pantalla.
60. **Cuando el usuario pregunte si mover/ajustar una posición o tamaño de UI/gameplay (una línea, un margen, un HUD) puede "romper con la resolución de algunos dispositivos", verificar PRIMERO `project.godot`'s `[display] window/stretch/mode`/`window/stretch/aspect` antes de responder por intuición.** Este proyecto usa `stretch/mode="canvas_items"` + `aspect="keep"` con un viewport lógico FIJO (390×844) — TODO el layout (`danger_line.gd`, `grid_math.gd`, `world_bounds.gd`, etc.) se calcula en ese espacio virtual, nunca en píxeles físicos de pantalla; Godot escala (con letterbox si hace falta) el resultado igual en cualquier dispositivo real. Esto significa que la pregunta correcta NO es "¿en qué dispositivo se ve mal?" sino "¿el nuevo valor sigue cabiendo dentro del propio lienzo virtual junto a los demás elementos?" (mismo tipo de chequeo que la regla #53 de auto-escalado con `min()` en ambos ejes). Verificado en la práctica: bajar la línea de peligro un bloque completo (`Constants.GRID_ROWS`) se confirmó como seguro calculando los márgenes reales en el espacio virtual (¿cuánto queda entre el nuevo borde del tablero y el molcajete?), sin necesitar un dispositivo físico ni emulador — la respuesta es matemática, no empírica, siempre que el proyecto use este stretch mode.
61. **Un obstáculo "indestructible por diseño" (nunca puede eliminarlo el jugador) combinado con un tablero/nivel que SIEMPRE avanza un paso por turno (se limpie o no ese paso) es un temporizador de derrota garantizado, no un caso raro — cualquier partida suficientemente larga termina apenas ese obstáculo alcance la condición de derrota, sin importar la habilidad del jugador.** Bug de diseño real reportado jugando (oleada 25, con captura): "una roca pasó la línea roja y perdí. Eso no debería pasar, porque las rocas no pueden ser destruidas." La Piedra de Molcajete es indestructible por TODOS los medios (daño normal, explosión de salsa, láser — los tres respetan el mismo guard `is_indestructible`), pero `_check_game_over()` no distinguía tipo de bloque: cualquier entrada en la matriz del tablero que llegara a la fila de derrota terminaba la partida igual que un bloque destructible. Como el tablero avanza SIEMPRE (nunca se "salta" un turno aunque no se limpie nada), esto no era un edge case — era el desenlace inevitable de CUALQUIER partida que durara lo suficiente como para que apareciera el obstáculo indestructible. Fix: el bloque indestructible desaparece (`queue_free()`, sin reinsertarse en la matriz) al cruzar la línea de derrota, en vez de contar para perder — mismo patrón que ya usan los power-ups/íconos que cruzan esa misma línea (ver regla de icons/danger-line de esta sesión). Regla general: al diseñar (o revisar) cualquier obstáculo "que nunca se puede quitar", verificar explícitamente qué pasa cuando llega al límite del área jugable — si la respuesta es "cuenta igual que cualquier otra pieza para la condición de derrota", es un bug de diseño esperando a que alguien juegue lo suficiente para encontrarlo, no algo que un playtest corto vaya a atrapar.
62. **Un `Button.text` es una sola cadena con un solo `font_color` — no puede tener partes de distinto color (ej. "Nombre" en un acento + "3/10 completados" en gris).** Cuando una tarjeta clickeable necesita contenido de varios colores/tamaños, la solución es dejar `Button.text = ""` (el Button sigue siendo el widget que recibe el toque y dibuja su `StyleBoxFlat`) y agregarle `Label` HIJOS posicionados a mano con sus propios `font_color`/`font_size` — pero cada uno de esos `Label` necesita `mouse_filter = Control.MOUSE_FILTER_IGNORE` explícito, si no absorben el toque en su propia área y la tarjeta deja de responder a taps que caigan exactamente sobre el texto (un bug de "a veces no responde" muy fácil de no notar en el editor con mouse, porque el mouse SÍ suele registrar el evento igual vía bubbling en algunos casos, pero un toque real en un dispositivo puede comportarse distinto según la jerarquía exacta — no asumir, poner el `mouse_filter` siempre que un Control se superponga a un widget clickeable padre). Verificar visualmente con captura real que la tarjeta completa (no solo el borde del botón) se ve con la jerarquía de color esperada.
...
- **Totopo:** HP central `= oleada`. **Queso:** HP central `= ceil(oleada * 1.5)`, daño x2, -15% velocidad de semilla al rebotar (piso `SEED_MIN_SPEED_RATIO = 0.35`). Modo Infinito ya NO usa ese HP central como valor fijo para todos los bloques de la fila — cada bloque lo sortea dentro de un rango que se ensancha por oleada (`WaveScalingGd.random_hp_for_wave()`, pedido explícito del usuario, ver regla de sesión en "Estado Actual del Juego"). **Salsa:** al morir, DESTRUYE (no daña) los 8 bloques pegados alrededor — cruz + diagonales (GDD actualizado, pedido explícito del usuario) — excepto piedra (exenta vía `is_indestructible`) y power-ups (viven en `_icons`, un Dictionary aparte que la explosión ni recorre). **Piedra:** indestructible por CUALQUIER medio (daño normal, salsa, láser) — al llegar a `Constants.MOLCAJETE_ROW` desaparece sin más efecto (`BoardManager._shift_down()`, guard genérico por `is_indestructible`, ver regla de sesión más abajo) en vez de terminar la partida, ya que el jugador nunca tiene forma de destruirla. Modo Nivel usa su propia escala VARIADA por NÚMERO DE NIVEL, ver `tools/gen_levels.py::random_totopo_hp()`.
