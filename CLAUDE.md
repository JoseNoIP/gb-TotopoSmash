...
- **Escenas jugables:** `MainMenu.tscn` (botones NIVELES / PACKS ESPECIALES / MODO INFINITO / TIENDA / CONFIGURACIÓN) → `LevelSelectScreen.tscn` (Modo Nivel, roster numérico + sección de packs al final) o `PackSelectScreen.tscn` → `PackLevelsScreen.tscn` (grilla de un solo pack, siempre desbloqueada — buzón `LevelManager.get_pending_pack_prefix()`) → `TutorialGame.tscn` (primera vez, cualquier modo) / `Game.tscn`. `Game.tscn` sirve ambos modos — el modo lo decide `LevelManager.get_pending_level()` (buzón no destructivo) leído en `Game.gd._ready()` justo antes de `GameManager.start_game(level_id)`.
- **Niveles:** `data/levels/*.json` + `data/levels/manifest.json` (orden de juego). Un nivel define su contenido con `cells` (celdas absolutas, visibles desde el inicio), `row_queue` (filas que se revelan de a una por turno, dificultad progresiva) y/o `static: true` (ver abajo) — el nivel normal (no-static) se gana cuando la cola se agota Y no queda ningún destructible. **108 niveles**: 100 numéricos (`tools/gen_levels.py`, objetivo del GDD) — 94 procedurales vía `row_queue`, nivel 1 = 10 filas totales y +3 filas por nivel siguiente hasta un TOPE de 50 filas [`total_rows_for_level()`, alcanzado en el nivel 15], + 6 figuras vía `cells` (Cruz/Corazón/Botella/Estrella/Diamante/Carita Feliz) — más 2 packs temáticos hand-authored con namespace propio (nunca `level_0NN`): `tools/gen_holiday_pack.py` (5 niveles, HP bajo tipo "figura") y `tools/gen_worldcup_pack.py` (**v2**, 3 niveles `static` de alta resolución — cancha/trofeo/"GOL" — ver abajo). Ampliable con la skill `/level-designer`. **`LevelSelectScreen` muestra los packs en una sección aparte ("PACKS ESPECIALES") SIEMPRE desbloqueada** — nunca gatearlos con `highest_level_unlocked` (esa regla es para la campaña numérica secuencial; un pack bonus/opcional no debería depender de terminar los 100 niveles primero). También hay una **pantalla dedicada** (`PackSelectScreen.tscn` → `PackLevelsScreen.tscn`, botón "PACKS" en `MainMenu`) para que los packs sean descubribles sin depender de hacer scroll.
...
- **Power-up láser** (`laser_icon.gd`, pedido explícito del usuario) — al tocarlo dispara `Constants.LASER_DAMAGE` a TODA la fila (horizontal) o columna (vertical) donde está, vía `EventBus.laser_triggered` → `BoardManager._on_laser_triggered()` (mismo patrón que la explosión en cruz de la salsa, pero en línea recta). Kind `"laser"` en `cell_factory.gd`/`wave_scaling.gd`, sin probabilidad de spawn en Modo Infinito (solo aparece en niveles autorados).
- **HP oculto en bloques muy chicos** (`block_base.gd`, `Constants.UI_MIN_READABLE_CELL_SIZE=20.0`) — en niveles `static` de alta resolución el número de HP sería ilegible y solo ensucia la figura; se oculta automáticamente por debajo del umbral, sin tocar nada más.
...
- **Build:** `gdlint` 0 errores · GUT 196/196 tests · `--export-debug Android` genera APK válido · `deploy-playstore.yml` probado (subió a Play Store Internal Testing).
