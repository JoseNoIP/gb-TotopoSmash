...
- **Niveles:** `data/levels/*.json` + `data/levels/manifest.json` (orden de juego). Un nivel define su contenido con `cells` (celdas absolutas, visibles desde el inicio — usado por las figuras) y/o `row_queue` (filas que se revelan de a una por turno, contenido fijo pero mecánica igual a Modo Infinito — usado por los niveles de dificultad progresiva; el nivel se gana solo cuando la cola se agota Y no queda ningún destructible). 100 niveles (`tools/gen_levels.py`, objetivo del GDD): 94 procedurales vía `row_queue`, nivel 1 = 10 filas totales y +3 filas por nivel siguiente hasta un TOPE de 50 filas [`total_rows_for_level()`, alcanzado en el nivel 15 — de ahí en adelante la dificultad escala por HP/variedad, no por duración], + 6 figuras vía `cells` — Cruz/Corazón/Botella/Estrella/Diamante/Carita Feliz. Ampliable con la skill `/level-designer`.
...
- **Build:** `gdlint` 0 errores · GUT 138/138 tests · `--export-debug Android` genera APK válido · `deploy-playstore.yml` probado (subió a Play Store Internal Testing).
