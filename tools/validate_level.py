...
KNOWN_KINDS = {"totopo", "queso", "salsa", "stone", "triangle", "lemon", "seed_extra"}
        ...
        if not isinstance(cells, list):
            ...
            for i, cell in enumerate(cells):
                if not isinstance(cell, dict):
                    ...
                    continue
                errors.extend(_validate_cell(cell, f"cells[{i}]", True, seen_positions))
            ...
            for r, row_cells in enumerate(row_queue):
                ...
                for i, cell in enumerate(row_cells):
                    if not isinstance(cell, dict):
                        ...
                        continue
                    errors.extend(
                        _validate_cell(cell, f"row_queue[{r}][{i}]", False, seen_cols)
                    )

    return errors


def _validate_cell(cell: dict, label: str, require_row: bool, seen_positions: set) -> list:
    errors = []

    col = cell.get("col")
    col_ok = _is_whole_number(col) and 0 <= int(col) < GRID_COLS
    if not col_ok:
        errors.append(f"{label}: 'col' fuera de rango [0, {GRID_COLS - 1}]")

    if require_row:
        row = cell.get("row")
        row_ok = _is_whole_number(row) and 0 <= int(row) <= MAX_CONTENT_ROW
        if not row_ok:
            errors.append(
                f"{label}: 'row' fuera de rango [0, {MAX_CONTENT_ROW}] (fila del molcajete prohibida)"
            )
